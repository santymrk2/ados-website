...
export const triggerRankingsRebuild = () => {
  ...
  rebuildTimeout = setTimeout(async () => {
    try {
      ...
      // Import and directly invoke the API handler to prevent network loopback issues in Docker
      const { GET: getActivities } = await import('../pages/api/activities');
      const actRes = await getActivities();
      if (!actRes.ok) return;
      const activitiesParsed = await actRes.json();
